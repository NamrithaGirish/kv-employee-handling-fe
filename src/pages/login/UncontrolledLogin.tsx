...
export const UncontrolledLogin = ()=>{
    ...
    const clearUsername = ()=>{
        ...
        clearButtonRef.current? clearButtonRef.current.disabled = true :false;
    }
                ...
                <div className="login-box">
                    <img src="./src/assets/kv-logo.png" alt="Keyvalue Logo" className="logo" />
                          <form className="form" onSubmit={handleRefLogin}>
                            <InputBox  text={"Username"} classname="username" onchange={(e:React.ChangeEvent<HTMLInputElement>)=>updateClearButton(e.target.value)} reference={usernameRef} name="username" endAdornment={<button ref={clearButtonRef} disabled={true} className="clear" type={"button"} onClick={clearUsername}>Clear</button>}/>
                            <InputBox text="Password" classname="textinput"reference={passwordRef} name="password" endAdornment={<Button text="clear" classname="clear" type={"button"} functionName={clearPassword}/>}/>
